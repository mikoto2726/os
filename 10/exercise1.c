...
int main(void) {
    ...
    while(1) {  // メインループ
        ...
        if (getnameinfo((struct sockaddr *)&their_addr, sin_size, NULL, 0, portstr, sizeof portstr, NI_NUMERICSERV) != 0) {
            fprintf(stderr, "getnameinfo failed\n");
            close(new_fd);
            continue;
        }

        printf("Your address is %s at port %s\n", ipstr, portstr);
